...
def parse_header(data: bytes) -> dict:
    ...
    return {
        ...
        # 이전 이름을 사용하는 코드와의 호환용
        "unknown_0c": auxiliary_count,
...
def build_entries(
    ...
    return entries, non_primary_secondary_indices
...
def analyze_nsf(
    ...
    entries, non_primary_secondary_indices = build_entries(
        data=data,
        blob_offset=calculated_blob_offset,
    ...
    if auxiliary_outside_blob:
        warnings.append(
            f"blob 범위 밖의 보조 테이블 값이 "
            f"{len(auxiliary_outside_blob)}개 있습니다."
        )
