#!/usr/bin/env python3
"""Independent postbuild review for the V7.15.12 PSP test ISO."""
...
ISO = ROOT / "workspace/build/prinny1_v7_15_12_larger_pic0_title/prinny_korean_v7_15_12_larger_pic0_title.iso"
...
OUTPUT = ROOT / "workspace/reports/prinny1_v7_15_12_larger_pic0_title_iso_review"
...
def main() -> int:
    for path, expected in EXPECTED.items():
        if not path.is_file() or sha256_file(path) != expected:
            raise ValueError(f"V7.15.12 사후 검토 입력 해시 불일치: {path}")
    ...
    offset, length = int(base_record["extent_lba"]) * SECTOR_SIZE, int(base_record["data_length"])
    ...
    if overlap_count(final_lzs) != 0 or hashlib.sha256(final_start).hexdigest() != "7d5a055eee55ff532f47e0f6ff6a56d842d0f9ed5bade41e4c15082b7a0e54e1":
    ...
    with Image.open(TITLE) as opened:
        ...
        if title.tobytes() != opened.convert("RGBA").tobytes():
            raise ValueError("최종 ISO 확대 PIC0 스타일 타이틀 PNG 왕복 불일치")
    ...
    report = {
        "format": "prinny1_v7_15_12_larger_pic0_title_iso_review_v1",
        ...
        "checks": {
            ...
            "larger_pic0_style_title_png_roundtrip_exact": True,
            "runtime_lzs_non_overlap": True,
            "prior_korean_images_preserved": True,
            "seven_zip_structure_retest": True,
        },
        "status": "pass_v7_15_12_iso_ready_for_ppsspp_runtime_test",
        "final_verdict": "PASS",
    }
    OUTPUT.mkdir(parents=True, exist_ok=True)
    (OUTPUT / "all_report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"ISO sha256: {sha256_file(ISO)}")
    print("SYSTEM-only/larger-PIC0-title/LZS/images/7z: PASS")
