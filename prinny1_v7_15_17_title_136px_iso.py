#!/usr/bin/env python3
"""Build the V7.15.17 136 px title-only test ISO."""
...
SYSTEM = ROOT / "workspace/build/prinny1_v7_15_17_title_136px_resources/SYSTEM.DAT"
REVIEW = ROOT / "workspace/reports/prinny1_v7_15_17_title_136px_review/all_report.json"
OUTPUT_DIR = ROOT / "workspace/build/prinny1_v7_15_17_title_136px"
OUTPUT_ISO = OUTPUT_DIR / "prinny_korean_v7_15_17_title_136px.iso"
REPORT_DIR = ROOT / "workspace/reports/prinny1_v7_15_17_title_136px_iso"
EXPECTED = {
    BASE_ISO: "f62aa240706b9830f7e1b46a8f707dcb3f9cf4cf6b476147667a162d43a1b7c6",
    SYSTEM: "23f05e52f4c013ba1f7a686b28c82be75831b64982f2f121efb488194b28b796",
    REVIEW: "1b86b455525cf0257c29aa1d26c20761204fcf6368406da1b3461fa8a9c21ffb",
}
...
def main() -> int:
    for path, expected in EXPECTED.items():
        if not path.is_file() or sha256_file(path) != expected:
            raise ValueError(f"V7.15.17 ISO 입력 해시 불일치: {path}")
    review = json.loads(REVIEW.read_text(encoding="utf-8"))
    if review.get("status") != "pass_v7_15_17_iso_build_ready_automatic_approval" or review.get("final_verdict") != "PASS":
        raise ValueError("V7.15.17 독립 사전 검토 미통과")
    ...
    if OUTPUT_ISO.exists():
        raise ValueError("V7.15.17 ISO가 이미 존재합니다. 덮어쓰지 않습니다.")
    ...
    if temporary.stat().st_size != BASE_ISO.stat().st_size:
        raise ValueError("V7.15.17 ISO 크기 변경")
    ...
    if test.returncode != 0 or "Everything is Ok" not in test.stdout:
        raise ValueError("V7.15.17 ISO 7z 구조 검사 실패")
    ...
    if read_iso_file(OUTPUT_ISO, find_iso_file(OUTPUT_ISO, ["PSP_GAME", "USRDIR", "SYSTEM.DAT"])) != final_system:
        raise ValueError("V7.15.17 SYSTEM.DAT 재추출 불일치")
    report = {
        "format": "prinny1_v7_15_17_title_136px_iso_v1",
        ...
        "status": "pass_v7_15_17_iso_built_independent_post_review_required",
    }
