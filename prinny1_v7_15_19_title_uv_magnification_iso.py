#!/usr/bin/env python3
"""Build the V7.15.19 title-only test ISO after independent review."""
...
SYSTEM = ROOT / "workspace/build/prinny1_v7_15_19_title_uv_magnification_resources/SYSTEM.DAT"
REVIEW = ROOT / "workspace/reports/prinny1_v7_15_19_title_uv_magnification_review/all_report.json"
OUTPUT_DIR = ROOT / "workspace/build/prinny1_v7_15_19_title_uv_magnification"
OUTPUT_ISO = OUTPUT_DIR / "prinny_korean_v7_15_19_title_uv_magnification.iso"
REPORT_DIR = ROOT / "workspace/reports/prinny1_v7_15_19_title_uv_magnification_iso"
EXPECTED = {
    BASE_ISO: "f62aa240706b9830f7e1b46a8f707dcb3f9cf4cf6b476147667a162d43a1b7c6",
    SYSTEM: "902ae0912a21d42b22d330c04ec3c046b4692218673a01205940a8dab63f4efe",
    REVIEW: "c4d3d1751fd705b7a1361c52df7c5f113aa40ae28c0f1b96a00faaa54cb4b38a",
}
...
def main() -> int:
    for path, expected in EXPECTED.items():
        if not path.is_file() or sha256_file(path) != expected:
            raise ValueError(f"V7.15.19 ISO 입력 해시 불일치: {path}")
    review = json.loads(REVIEW.read_text(encoding="utf-8"))
    if review.get("status") != "pass_v7_15_19_iso_build_ready_automatic_approval" or review.get("final_verdict") != "PASS":
        raise ValueError("V7.15.19 독립 사전 검토 미통과")
    ...
    if OUTPUT_ISO.exists():
        raise ValueError("V7.15.19 ISO가 이미 존재합니다. 덮어쓰지 않습니다.")
    ...
    if temporary.stat().st_size != BASE_ISO.stat().st_size:
        raise ValueError("V7.15.19 ISO 크기 변경")
    ...
    if test.returncode != 0 or "Everything is Ok" not in test.stdout:
        raise ValueError("V7.15.19 ISO 7z 구조 검사 실패")
    ...
    if read_iso_file(OUTPUT_ISO, find_iso_file(OUTPUT_ISO, ["PSP_GAME", "USRDIR", "SYSTEM.DAT"])) != final_system:
        raise ValueError("V7.15.19 SYSTEM.DAT 재추출 불일치")
    report = {
        "format": "prinny1_v7_15_19_title_uv_magnification_iso_v1",
        ...
        "status": "pass_v7_15_19_iso_built_independent_post_review_required",
    }
